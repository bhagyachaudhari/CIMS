...
public class FeesReceipt extends JInternalFrame implements ActionListener
{
	Container cp;
		...
		if(ae.getSource()==bsv)
		{
			...
			try
			{
				...
				Statement st1=con.createStatement();
                                   		ResultSet rs1=st1.executeQuery("select * from receipt");
                                  		while(rs1.next())
                                 		{
                                   			int no=rs1.getInt(1);
                                   			no++;
			...
			try
			{
				...
				if(rs.next())
				{
					...
					cb2.setSelectedItem(rs.getString(9));
						...
						Statement st1=con.createStatement();
						ResultSet rs1=st1.executeQuery("select * from admission where Student_ID="+Integer.parseInt(s));  //+Integer.parseInt(s)
						if(rs1.next())
						{
							trs.setText(""+rs1.getString(7));
						}
				}
				else
				{
