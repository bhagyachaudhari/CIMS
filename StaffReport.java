...
import java.sql.*;
import java.util.*;
public class  StaffReport  extends JInternalFrame implements ActionListener 
{
	...
	StaffReport()
	{
		...
		setClosable(true);	
		setSize(885,540);
		setTitle("Student Details Report");
	}
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==print)
		{
			...
			if(op==JFileChooser.APPROVE_OPTION)
			{
				...
				s="\n"+"\t"+"\t"+"\t"+"\t"+s+"\n"+"ID"+" "+"Staff_Code"+" "+"Staff_Name"+" "+"Gender"+" "+"Qualification"+" "+"Address"+" "+"PhoneNo"+"Designation"+" "+"Faculty"+" "+"Salary"+"\n";
		...
		if(e.getSource()==show)
		{
			try
			{
				...
				ResultSet rs=st.executeQuery("select Staff_ID,Staff_Code,Staff_Name,Gender,Qualification,Address,Phone_No,Designation,Faculty,Salary from staff");
		...
		if(e.getSource()==search)
		{
			...
			try
			{
				...
				Statement st=con.createStatement();
				ResultSet rs=st.executeQuery("select Staff_ID,Staff_Code,Staff_Name,Gender,Qualification,Address,Phone_No,Designation,Faculty,Salary from staff where Staff_ID="+tsearch.getText());
				ResultSetMetaData rsmd=rs.getMetaData();
